            ...
            <ul className="horizontal sm-menu menu items-stretch px-3 shadow-lg bg-base-100 rounded-box w-full sm:w-auto sm:float-right overflow-visible">
                ...
                <li>
                    <div className="dropdown dropdown-end">
                        <button onClick={async () => {await promptUserForEmail(); document.getElementById('download').classList.toggle('modal-open')}} className="btn btn-link text-base text-base-content hover:bg-gray-200 hover:no-underline font-normal normal-case rounded-0" style={{ height: '100%', borderRadius: "0px" }} tabIndex="0">Download GWAS</button>
            ...
            <div id="download" className="modal">
                <div className="modal-box">
                    <div className="flex flex-col w-full">
                        ...
                        <p><a onClick={async () => { await promptUserForEmail(); setisC32Downloading(true); await downloadSet("C32_", setC32TotalFiles, setC32CurrentFile, userEmail); setisC32Downloading(false)}} className="btn btn-primary btn-block">{isC32Downloading? "Downloading, please wait...": "Download C32 GWAS"}</a></p>
                        <p hidden={!isC32Downloading}>Downloading... (File {C32CurrentFile}/{C32TotalFiles})</p>
                        <p><a onClick={async () => { await promptUserForEmail(); setisC64Downloading(true); await downloadSet("C64_", setC64TotalFiles, setC64CurrentFile, userEmail); setisC64Downloading(false)}} className="btn btn-primary btn-block"> {isC64Downloading? "Downloading, please wait...": "Download C64 GWAS"}</a></p>
                        <p hidden={!isC64Downloading}>Downloading... (File {C64CurrentFile}/{C64TotalFiles})</p>
                        <p><a onClick={async () => { await promptUserForEmail(); setisC128Downloading(true); await downloadSet("C128_", setC128TotalFiles, setC128CurrentFile, userEmail); setisC128Downloading(false)}} className={"btn btn-primary btn-block "}>{isC128Downloading? "Downloading, please wait...": "Download C128 GWAS"}</a></p>
                        <p hidden={!isC128Downloading}>Downloading... (File {C128CurrentFile}/{C128TotalFiles})</p>
                        <p><a onClick={async () => { await promptUserForEmail(); setisC256Downloading(true); await downloadSet("C256_", setC256TotalFiles, setC256CurrentFile, userEmail); setisC256Downloading(false)}} className={"btn btn-primary btn-block "}>{isC256Downloading? "Downloading, please wait...": "Download C256 GWAS"}</a></p>
                        <p hidden={!isC256Downloading}>Downloading... (File {C256CurrentFile}/{C256TotalFiles})</p>
                        <p><a onClick={async () => { await promptUserForEmail(); setisC512Downloading(true); await downloadSet("C512_", setC512TotalFiles, setC512CurrentFile, userEmail); setisC512Downloading(false)}} className={"btn btn-primary btn-block "}>{isC512Downloading? "Downloading, please wait...": "Download C512 GWAS"}</a></p>
                        <p hidden={!isC512Downloading}>Downloading... (File {C512CurrentFile}/{C512TotalFiles})</p>
                        <p><a onClick={async () => { await promptUserForEmail(); setisC1024Downloading(true); await downloadSet("C1024_", setC1024TotalFiles, setC1024CurrentFile, userEmail); setisC1024Downloading(false)}} className={"btn btn-primary btn-block "}>{isC1024Downloading? "Downloading, please wait...": "Download C1024 GWAS"}</a></p>
